...
public final class KrakkExplosionRuntime implements KrakkExplosionApi {
    ...
    private static final double ENTITY_HEALTH_OBSIDIAN_EQUIVALENT = 500.0D;
    ...
    private static void detonate(ServerLevel level, double x, double y, double z, Entity source, LivingEntity owner,
                                 ...
                                 boolean applyWorldChanges, boolean emitEffects, ExplosionProfileTrace trace) {
        if (emitEffects) {
            level.playSound(
                    null,
                    x,
        ...
        ExplosionEntityContext entityContext = applyWorldChanges
                ? buildExplosionEntityContext(level, x, y, z, blastRadius)
                : null;
        Long2DoubleOpenHashMap blockImpacts = collectRaycastImpacts(
                level, x, y, z, blastRadius, blastPower, random, emitEffects, trace, entityContext
        );
        BlockPos.MutableBlockPos mutablePos = new BlockPos.MutableBlockPos();
        for (Long2DoubleMap.Entry entry : blockImpacts.long2DoubleEntrySet()) {
            long blockPosLong = entry.getLongKey();
        ...
        applyEntityImpacts(level, source, owner, entityContext, applyWorldChanges);
    ...
    private static Long2DoubleOpenHashMap collectRaycastImpacts(ServerLevel level, double centerX, double centerY, double centerZ,
        ...
        Long2DoubleOpenHashMap impacts = new Long2DoubleOpenHashMap(Math.max(256, rayCount * 8));
        if (trace != null) {
            trace.initialRays = rayCount;
        }
        double baseAngularSpacingSquared = (4.0D * Math.PI) / Math.max(1, rayCount);
        double splitDistanceThresholdSquared = RAY_SPLIT_DISTANCE_THRESHOLD * RAY_SPLIT_DISTANCE_THRESHOLD;
        ...
        while (!rayQueue.isEmpty()) {
            ...
            while (ray.energy > 0.0D) {
                ...
                if (ray.travelT + 1.0E-9D >= ray.nextSplitCheckT) {
                    if (KrakkRaySplitMath.shouldSplitRay(
                            ray.splitDepth,
                            MAX_RAY_SPLIT_DEPTH,
                            ray.energy,
                            ...
                            ray.x, ray.y, ray.z,
                            centerX, centerY, centerZ,
                            baseAngularSpacingSquared,
                            splitDistanceThresholdSquared
                    )) {
                        splitRay(ray, ray.x, ray.y, ray.z, ray.energy, rayQueue, random, trace, entityContext);
                        break;
                    }
                    ray.nextSplitCheckT = ray.travelT + RAY_STEP;
                }
                ...
                boolean splitDuringAdvance = false;
                double sampleT = ray.nextSplitCheckT;
                if (sampleT < ray.travelT) {
                    double behind = ray.travelT - sampleT;
                    sampleT += (Math.floor(behind / RAY_STEP) + 1.0D) * RAY_STEP;
                }
                double dt = sampleT - ray.travelT;
                double sampleX = ray.x + (ray.dirX * dt);
                double sampleY = ray.y + (ray.dirY * dt);
                double sampleZ = ray.z + (ray.dirZ * dt);
                double sampleEnergy = ray.energy - (dt * RAY_DECAY_PER_UNIT);
                double sampleStepX = ray.dirX * RAY_STEP;
                double sampleStepY = ray.dirY * RAY_STEP;
                double sampleStepZ = ray.dirZ * RAY_STEP;
                while (sampleT <= nextT + 1.0E-9D) {
                    if (sampleEnergy <= 0.0D) {
                        break;
                    }
                    if (KrakkRaySplitMath.shouldSplitRay(
                            ray.splitDepth,
                        ...
                        splitRay(ray, sampleX, sampleY, sampleZ, sampleEnergy, rayQueue, random, trace, entityContext);
                        splitDuringAdvance = true;
                        break;
                    }
                    sampleT += RAY_STEP;
                    sampleX += sampleStepX;
                    sampleY += sampleStepY;
                    sampleZ += sampleStepZ;
                    sampleEnergy -= RAY_DECAY_PER_STEP;
                }
                ray.nextSplitCheckT = sampleT;
                if (splitDuringAdvance) {
                    break;
                }
        ...
        if (trace != null) {
            trace.rawImpactedBlocks = impacts.size();
        }
        Long2DoubleOpenHashMap antiAliasedImpacts = applyLightRaycastAntialiasing(level, impacts);
        if (trace != null) {
            trace.postAaImpactedBlocks = antiAliasedImpacts.size();
        }
    ...
    private static ExplosionEntityContext buildExplosionEntityContext(ServerLevel level,
        ...
        List<EntityHitProxy>[] octants = new List[8];
        for (int i = 0; i < octants.length; i++) {
            octants[i] = new ArrayList<>();
        }
        List<EntityHitProxy> shared = new ArrayList<>();
        ...
        for (LivingEntity entity : candidates) {
            ...
            boolean spansX = aabb.minX <= centerX && aabb.maxX >= centerX;
            boolean spansY = aabb.minY <= centerY && aabb.maxY >= centerY;
            boolean spansZ = aabb.minZ <= centerZ && aabb.maxZ >= centerZ;
            if (spansX || spansY || spansZ) {
                shared.add(proxy);
            } else {
                int octant = octantForPoint(
        ...
        return new ExplosionEntityContext(octants, shared, all);
    }
    ...
    private static double applyEntitySegmentInteractions(ExplosionEntityContext context,
        ...
        int octant = octantForDirection(ray.dirX, ray.dirY, ray.dirZ);
        List<EntityHitProxy> primary = context.octants[octant];
        List<EntityHitProxy> shared = context.shared;
        if (primary.isEmpty() && shared.isEmpty()) {
        ...
        while (remainingDistance > 1.0E-6D && segmentStartEnergy > 0.0D) {
            ...
            for (EntityHitProxy proxy : primary) {
            ...
            for (EntityHitProxy proxy : shared) {
                if (proxy.lastRayId == ray.rayId || !proxy.entity.isAlive()) {
                    continue;
                }
                double hitDistance = intersectRaySegmentAabbDistance(
    ...
    private static void applyEntityImpacts(ServerLevel level,
        ...
        for (EntityHitProxy proxy : context.all) {
            if (proxy.accumulatedImpact <= ENTITY_MIN_APPLIED_IMPACT || !proxy.entity.isAlive()) {
                continue;
            }

            double maxHealth = Math.max(1.0D, proxy.entity.getMaxHealth());
            double absorption = Math.max(0.0D, proxy.entity.getAbsorptionAmount());
            double strength = ENTITY_HP_STRENGTH_BASE + ((maxHealth + absorption) * ENTITY_HP_STRENGTH_PER_HP);
            double rawDamage = proxy.accumulatedImpact / Math.max(strength, 1.0E-6D);
            double armorDivisor = 1.0D
                    + (proxy.entity.getArmorValue() * ENTITY_ARMOR_DAMAGE_DIVISOR)
                    + (proxy.entity.getAttributeValue(Attributes.ARMOR_TOUGHNESS) * ENTITY_TOUGHNESS_DAMAGE_DIVISOR);
            ...
            proxy.entity.hurt(explosionDamage, (float) finalDamage);
            if (!proxy.entity.isAlive()) {
                continue;
            }

            double pressureMagnitudeSq = (proxy.pressureX * proxy.pressureX)
                    + (proxy.pressureY * proxy.pressureY)
                    + (proxy.pressureZ * proxy.pressureZ);
            if (pressureMagnitudeSq <= 1.0E-8D || !Double.isFinite(pressureMagnitudeSq)) {
                continue;
            }
            double pressureMagnitude = Math.sqrt(pressureMagnitudeSq);
            double knockbackResistance = Mth.clamp(proxy.entity.getAttributeValue(Attributes.KNOCKBACK_RESISTANCE), 0.0D, 1.0D);
            ...
            double knockbackX = proxy.pressureX * invMagnitude * knockbackStrength;
            double knockbackY = (proxy.pressureY * invMagnitude * knockbackStrength)
                    + (knockbackStrength * ENTITY_KNOCKBACK_UPWARD_BIAS);
            double knockbackZ = proxy.pressureZ * invMagnitude * knockbackStrength;
            if (!Double.isFinite(knockbackX) || !Double.isFinite(knockbackY) || !Double.isFinite(knockbackZ)) {
                continue;
            }
            proxy.entity.push(knockbackX, knockbackY, knockbackZ);
            proxy.entity.hurtMarked = true;
        }
    }
    ...
    private static int octantForDirection(double dirX, double dirY, double dirZ) {
        int octant = 0;
        if (dirX >= 0.0D) {
            octant |= 1;
        }
    ...
    private static int octantForPoint(double x, double y, double z, double centerX, double centerY, double centerZ) {
        int octant = 0;
        if (x >= centerX) {
            octant |= 1;
        }
    ...
    private static Long2DoubleOpenHashMap applyLightRaycastAntialiasing(ServerLevel level, Long2DoubleOpenHashMap source) {
        ...
        for (Long2DoubleMap.Entry entry : source.long2DoubleEntrySet()) {
            ...
            int posX = BlockPos.getX(posLong);
            int posY = BlockPos.getY(posLong);
            int posZ = BlockPos.getZ(posLong);
            ...
            result.addTo(posLong, retainedImpact);
            ...
            candidateCount = collectAaNeighbor(level, mutablePos, neighborKeys, neighborWeights, candidateCount, posX + 1, posY, posZ);
            candidateCount = collectAaNeighbor(level, mutablePos, neighborKeys, neighborWeights, candidateCount, posX - 1, posY, posZ);
            candidateCount = collectAaNeighbor(level, mutablePos, neighborKeys, neighborWeights, candidateCount, posX, posY + 1, posZ);
            candidateCount = collectAaNeighbor(level, mutablePos, neighborKeys, neighborWeights, candidateCount, posX, posY - 1, posZ);
            candidateCount = collectAaNeighbor(level, mutablePos, neighborKeys, neighborWeights, candidateCount, posX, posY, posZ + 1);
            candidateCount = collectAaNeighbor(level, mutablePos, neighborKeys, neighborWeights, candidateCount, posX, posY, posZ - 1);
            ...
            if (candidateCount <= 0 || weightTotal <= 1.0E-8D) {
                result.addTo(posLong, spreadImpact);
                continue;
            }
    ...
    private static void splitRay(RaycastState parent,
                                 ...
                                 ExplosionEntityContext entityContext) {
        double splitEnergy = parentEnergy * 0.5D;
        if (splitEnergy <= 0.0D) {
            return;
        }
        if (trace != null) {
        ...
        int rightRayId = entityContext != null ? entityContext.nextRayId() : parent.rayId;
        int leftRayId = entityContext != null ? entityContext.nextRayId() : parent.rayId;
    ...
    private static final class ExplosionEntityContext {
        private final List<EntityHitProxy>[] octants;
        private final List<EntityHitProxy> shared;
        private final List<EntityHitProxy> all;
        private int nextRayId = 1;

        private ExplosionEntityContext(List<EntityHitProxy>[] octants, List<EntityHitProxy> shared, List<EntityHitProxy> all) {
    ...
    public void detonate(ServerLevel level, double x, double y, double z, Entity source, LivingEntity owner, KrakkExplosionProfile profile) {
        ...
        detonate(level, x, y, z, source, owner, resolution.radius, resolution.power, level.random, true, true, null);
    }
    ...
    public ExplosionProfileReport profileDetonate(ServerLevel level, double x, double y, double z, Entity source,
        ...
        detonate(level, x, y, z, source, owner, resolution.radius, resolution.power, random, applyWorldChanges, applyWorldChanges, trace);
    ...
    private static ExplosionResolution resolveProfile(KrakkExplosionProfile profile) {
        ...
        if (profile == null) {
            ...
            resolvedRadius = KrakkExplosionCurves.computeBlastRadius(resolvedPower);
        } else {
            resolvedPower = KrakkExplosionCurves.sanitizeImpactPower(profile.impactPower());
            resolvedRadius = Math.max(0.1D, profile.blastRadius());
        }
